...
class User extends CI_Controller{
    ...
    public function insertUser($user_info):array{
        ...
        //檢查是否有重複帳號
        $search_user_info = $this->user_model->searchUser($user_info['account']);
        if($search_user_info->num_rows() <= 0){
            $user_id = $this->user_model->insertUsers($user_info);
            if(!$user_id){
                $message['status'] = 0;
     ...
     * @return array
     */
    public function searchUser($account = null):array{
        $user_list = $this->user_model->searchUser($account)->row_array();
        return $user_list;
    }
}