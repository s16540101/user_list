...
class Worksheet implements IComparable
{
   ...
   /**
    ...
    private static function checkSheetTitle($pValue)
    {
        ...
        // Maximum 31 characters allowed for sheet title
        if (Shared\StringHelper::countCharacters($pValue) > 100) {
    ...
    public function setTitle($pValue = 'Worksheet', $updateFormulaCellReferences = true)
    {
        ...
        // Syntax check
        $pValue = str_replace(self::$invalidCharacters, ' ', $pValue); // 20190930 將不允許設定到工作區名稱的字元換成空白
    ...
    */
     ...
     * @param string|null $dateFormat 若資料是日期類型，就強制轉換其格式 ex: Y/m/d H:i:s
     * @return array
     */
    public function rangeToArray($pRange = 'A1', $nullValue = null, $calculateFormulas = true, $formatData = true, $returnCellRef = false, $dateFormat=null)
    {
        // Returnvalue
        $returnValue = array();
            ...
            for ($col = $minCol; $col != $maxCol; ++$col) {
                ...
                if ($this->cellCollection->isDataSet($col.$row)) {
                    ...
                    if ($cell->getValue() !== null) {
                        ...
                        if ($formatData) {
                            if (!is_null($dateFormat) && \PHPExcel\Shared\Date::isDateTime($cell)) {
                                // if cell's value type is datetime, and $dateFormat is set, format cell value to $dateFormat
                                $dateObj = \PHPExcel\Shared\Date::excelToPHPObject($returnValue[$rRef][$cRef]);
                                $returnValue[$rRef][$cRef] = $dateObj->format($dateFormat);
                            } else {
